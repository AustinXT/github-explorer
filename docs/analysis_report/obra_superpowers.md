...
将 AI 编程助手从随意的代码生成器转变为纪律严明的高级工程师——通过心理学工程化的强制性工作流，而非建议性的 prompt 技巧。
...
Jesse 观察到一个被多数人忽视的问题：AI 编程助手的**行为纪律**，而非能力，才是生产环境中的瓶颈。AI 不是不会写测试——它是在压力下（上下文膨胀、任务复杂、用户催促）选择跳过测试。这是一个行为工程问题，不是能力问题。
...
4. **CSO（Claude Search Optimization）方法论**（新颖度 4/5 × 实用性 4/5）
   - 关键反直觉发现：如果技能描述包含工作流摘要，AI 会直接按描述执行而跳过完整技能内容。因此描述必须只包含触发条件，绝不包含工作流步骤
...
1. **技能以纯 Markdown 实现，而非代码/DSL**
   ...
   - 这是整个项目最大胆的赌注——证明了精心编写的自然语言文档可以成为有效的 AI 行为约束
...
- **信息差**: 无传统意义上的信息差（103K stars 已是顶流），但**方法论层面**存在信息差——多数用户只安装了插件，未深入理解背后的心理学工程方法和 TDD-for-docs 方法论，这些方法论本身的价值远超插件功能
