...
| 贡献模式 | 创始人主导 + 核心团队（666ghj 403 次 + MaYiding 301 次，占 77%） |
...
|------|---------|--------|--------|--------|
| 定位 | 舆情分析专用多 Agent | 通用 AI Agent 框架 | 通用多 Agent 对话框架 | 企业级舆情 SaaS |
| 数据采集 | 内置 MindSpider 爬虫 | 无内置 | 无内置 | 商业数据源 |
| 多模态 | 视频截图分析+结构化卡片 | 有限 | 文本为主 | 文本+图片 |
| 报告生成 | IR 驱动+6 套模板+HTML/PDF | 无 | 无 | 固定模板 |
| 部署难度 | 高（7 个 LLM API+数据库） | 中等 | 中等 | 即开即用 |
| 定制成本 | 低（纯 Python 易改） | 低 | 中等 | 高（闭源 SaaS） |
...
| 技术评测 | [LLM 应用剖析: 舆情分析多智能体-微舆 BettaFish](https://www.cnblogs.com/mengrennwpu/p/19222764) |
